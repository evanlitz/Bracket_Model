"""Scrape player stats for all teams, 2002-2026 (excludes 2020).

Output: data/{year}/players/{slug}_players.parquet  (one file per team-year)
"""
import random
import sys
import time
...
from src.scouting import fetch_team_page, parse_player_table
from src.kenpom import load_all_kenpom

YEARS = list(range(2002, 2027))
YEARS.remove(2020)

LOG_PATH = Path('scrape_players_errors.log')
...
for i, (team, year) in enumerate(team_years, 1):
    out_dir = Path('data') / str(year) / 'players'
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / f'{team_slug(team)}_players.parquet'
    ...
    try:
        ...
        players = parse_player_table(html, team, year)

        if not players:
            log_error(f'EMPTY {year} {team}: no player rows parsed')
        else:
            pd.DataFrame(players).to_parquet(out_path, index=False)
            print(f'[{i}/{len(team_years)}] OK   {year} {team}  ({len(players)} players)')
